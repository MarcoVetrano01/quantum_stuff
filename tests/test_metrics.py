...
class TestFidelity:
    def test_identical_pure_states(self):
        ...
        psi = zero()
        rho1 = ket_to_dm(psi, batchmode=False)
        rho2 = ket_to_dm(psi, batchmode=False)
        f = fidelity(rho1, rho2)
        assert np.isclose(f, 1.0)
    
    def test_orthogonal_pure_states(self):
        """Test fidelity between orthogonal pure states"""
        rho1 = ket_to_dm(zero(), batchmode = False)
        rho2 = ket_to_dm(one(), batchmode = False)
        f = fidelity(rho1, rho2)
        assert np.isclose(f, 0.0)
    
    def test_superposition_states(self):
        """Test fidelity between superposition states"""
        rho1 = ket_to_dm(plus(), batchmode = False)
        rho2 = ket_to_dm(minus(), batchmode = False)
        f = fidelity(rho1, rho2)
    ...
    def test_fidelity_bounds(self):
        ...
        for i, psi1 in enumerate(states):
            for j, psi2 in enumerate(states):
                rho1 = ket_to_dm(psi1, batchmode = False)
                rho2 = ket_to_dm(psi2, batchmode = False)
                f = fidelity(rho1, rho2)
                assert 0 <= f <= 1
...
class TestTraceDistance:
    def test_identical_states(self):
        ...
        rho = ket_to_dm(zero(), batchmode = False)
...
class TestVonNeumannEntropy:
    ...
    def test_entropy_non_negative(self):
        ...
        for p in np.linspace(0, 1, 11):
            rho = p * ket_to_dm(zero(), batchmode = False) + (1-p) * ket_to_dm(one(), batchmode = False)
            S = von_neumann_entropy(rho)
    ...
    def test_bell_state_entropy(self):
        ...
        rho = ket_to_dm(bell, batchmode = False)
        S = von_neumann_entropy(rho)
...
class TestMutualInformation:
    def test_product_state_mutual_info(self):
        ...
        rho = ket_to_dm(psi_product, batchmode = False)
